...
# Tracks applied migrations in _prisma_migrations (Prisma's own table).
# ──────────────────────────────────────────────────────────────────────────────
...
async function run() {
  ...
  for (const name of dirs) {
    ...
    try {
      ...
      await client.query(
        'INSERT INTO \"_prisma_migrations\" (id, checksum, migration_name, finished_at, applied_steps_count) VALUES (\$1, \$2, \$3, NOW(), 1) ON CONFLICT (id) DO NOTHING',
        [id, checksum, name]
      );
      console.log('[startup] Applied:', name);
    } catch (e) {
      // ALTER TYPE ... ADD VALUE IF NOT EXISTS is safe to re-run but pg throws
      // if the value already exists without IF NOT EXISTS in older SQL.
